...
@test "Should run docker container and be able to login via ssh as root user and execute echo \"whoami\" command" {
    ...
    # copy ssh keys
    sudo docker cp $DOCKER_CONTAINER_ID:/root/.ssh/id_rsa $BATS_TMPDIR/root_keys/id_rsa
    echo "Print root keys directory"  >&3
    ls -la $BATS_TMPDIR/root_keys >&3
    # change owner
    sudo chown $(whoami):$(whoami) $BATS_TMPDIR/root_keys/id_rsa
    chmod 600 $BATS_TMPDIR/root_keys/id_rsa
    ...
    run ssh -i $BATS_TMPDIR/root_keys/id_rsa -o "StrictHostKeyChecking=no" -o "PasswordAuthentication=no" -l root $DOCKER_CONTAINER_HOSTNAME whoami >&3
